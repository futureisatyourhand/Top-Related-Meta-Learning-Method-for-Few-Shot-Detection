...
from utils import *
from loss import RankingLossFunc,KL_LossFunc
from cfg import cfg
from numbers import Number
from random import random
from fegment import SoftIoULoss
import pdb
...
def build_targets(pred_boxes, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen,conf):
    nB = target.size(0)
    nA = num_anchors
    nC = num_classes
    conf=conf.view(nB*nA*nH*nW,)
    ...
    for b in xrange(nB):
        ...
        cur_conf=conf[b*nAnchors:(b+1)*nAnchors]
        ...
        for t in xrange(cfg.max_boxes):
            ...
            #print(bbox_ious(cur_pred_boxes, cur_gt_boxes, x1y1x2y2=False))
            ii=bbox_ious(cur_pred_boxes, cur_gt_boxes, x1y1x2y2=False)#*cur_conf.data.cpu()
            cur_ious = torch.max(cur_ious,ii)
    ...
    for b in xrange(nB):
        ...
        for t in xrange(50):
            ...
            cf=conf[b*nAnchors+best_n*nPixels+gj*nW+gi]
            ...
            #####classification*iou
            #print(iou)
            #iou=iou*cf.data.cpu().numpy()[0]
            tconf[b][best_n][gj][gi] = iou
            tcls[b][best_n][gj][gi] =int(target[b][t*5])
            if iou > 0.5:
                nCorrect = nCorrect + 1
...
class RegionLossV2(nn.Module):
    ...
    """
    def __init__(self, num_classes=0, anchors=[],weights=[], num_anchors=1):
    ...
    def forward(self, output,meta, target,savetrain):
        ...
        #print(output.shape,target.shape)#((1280L, 30L, 13L, 13L), (64L, 20L, 250L)) 
        cls = output.view(output.size(0), nA, (5+nC), nH, nW)#batchfsize*classes,5,5+1,13,13
        #print(output.shape,cls.shape,target.shape)
        cls = cls.index_select(2, Variable(torch.linspace(5,5+nC-1,nC).long().cuda())).squeeze()#batchfsize*classes,5,1,13,13
        
        cls = cls.view(bs, cs, nA*nC*nH*nW).transpose(1,2).contiguous().view(bs*nA*nC*nH*nW, cs)##batchfsize,classes,5*1*13*13

        ###reweight loss
        #meta_channel=meta.shape[1]
        #meta=meta.view(-1,cs,meta_channel)##shape:(classes,num_gpus,1024)
        
        #kl_loss=self.KLloss(meta)
        loss_meta=self.meta_scale*self.KLloss(meta).cuda()###############################
        ...
        #pred_boxes[4]=torch.Tensor(conf).cuda().view(nB*nA*nH*nW)
        ...
        nGT, nCorrect, coord_mask, conf_mask, cls_mask, tx, ty, tw, th, tconf,tcls = build_targets(pred_boxes, target.data, self.anchors, nA, nC, \
                                                               nH, nW, self.noobject_scale, self.object_scale, self.thresh, self.seen,conf)
        ...
        tw    = Variable(tw.cuda(),requires_grad=False)
        th    = Variable(th.cuda(),requires_grad=False)
        tconf = Variable(tconf.cuda(),requires_grad=False)
        ...
        loss_cls=self.class_scale*self.classification(cls,tcls)################
        
        #ids = [9,11,12,16]
        #new_cls, new_tcls,flag = select_classes(cls, tcls, ids)
        #if flag==1:
        #    new_tcls = Variable(torch.from_numpy(new_tcls).long().cuda())
        #    loss_cls_new = self.class_scale * self.classification(new_cls, new_tcls)
        #    loss_cls_new*=10
        #    loss_cls += loss_cls_new



        loss = loss_x + loss_y + loss_w + loss_h + loss_conf + loss_cls+loss_meta
        with open('./backup/'+savetrain,'a+') as f:
            f.write(str(nGT)+" "+str(nCorrect)+" "+str(nProposals)+" "+\
                    str(loss_x.data[0])+" "+str(loss_y.data[0])+" "+\
                    str(loss_w.data[0])+" "+str(loss_h.data[0])+" "+\
                    str(loss_conf.data[0])+" "+str(loss_cls.data[0])+" "+str(loss.data[0])+" "+str(loss.data[0])+'\n')
        f.close()
        ...
        if False:
            ...
            print('             total : %f' % (t4 - t0))
        print('%d:nGT:%d,recall:%d,proposals:%d,loss=x:%f,y:%f,w:%f,h:%f,conf:%f,cls:%f,meta:%f,total:%f' % (self.seen, nGT, \
                   nCorrect, nProposals, loss_x.data[0], loss_y.data[0], loss_w.data[0], loss_h.data[0], loss_conf.data[0],\
                                  loss_cls.data[0],loss_meta.data[0],loss.data[0]))
