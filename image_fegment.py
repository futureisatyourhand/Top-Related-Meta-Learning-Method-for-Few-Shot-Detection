...
def data_augmentation(img, shapes, jitter, hue, saturation, exposure, flag=True):
    oh = img.height  
    ow = img.width
    ...
    for wh in shapes:        
        ...
        sxs.append(sx)
        sys.append(sy)
    return imgs, flips, dxs,dys,sxs,sys 
...
def fill_truth_detection_fegment(labpath,shape):
    max_boxes = cfg.max_boxes
    n_cls = len(cfg.base_classes)
    label = np.zeros((n_cls, max_boxes, 5))
    oh = img.height  
...
def load_data_detection(imgpath, labpath,shape, jitter, hue, saturation, exposure, data_aug=True):
    ...
    if cfg.metayolo:
        label = fill_truth_detection_meta(labpath, img[0].width, img[0].height, flip[0], dx[0], dy[0], 1./sx[0], 1./sy[0])
    else:
        label = fill_truth_detection(labpath, img[0].width, img[0].height, flip[0], dx[0], dy[0], 1./sx[0], 1./sy[0])
    if cfg.fegment:
        feg=fill_truth_detection_fegment(labpath,img,shape)
    else:
        feg=None
    return img,label,feg

def load_data_with_label(imgpath, labpath, shape, jitter, hue, saturation, exposure, data_aug=True):
    ## data augmentation
    img = Image.open(imgpath).convert('RGB')
    img,flip,dx,dy,sx,sy = data_augmentation(img, shape, jitter, hue, saturation, exposure, flag=data_aug)
    label = load_label(labpath, img[0].width, img[0].height, flip[0], dx[0], dy[0], 1./sx[0], 1./sy[0])
    return img[0], label
