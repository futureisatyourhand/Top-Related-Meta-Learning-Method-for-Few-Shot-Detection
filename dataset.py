...
def topath(p):
    return p.replace('/scratch/bykang/datasets', '/home1/liqian/data/voc')#coco2014')
...
class listDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img, label = load_data_detection(imgpath, labpath, self.fpns, jitter, hue, saturation, exposure, data_aug=self.train)
        ...
        #rgbs=torch.FloatTensor([[[0.25]],[[0.25]],[[0.5]]])
    ...
    def get_labpath(imgpath):
        ...
        labpath = imgpath.replace('images', subdir) \
                         ...
                         .replace('.jpg', '.txt').replace('.png','.txt').replace('train2014/','fewlist/').replace('val2014/','fewlist/')
       
        return labpath#.replace('train2014/','fewlist/')
...
class MetaDataset(Dataset):
    def __init__(self,
        ...
        self.inds = sum(metainds, []) if ensemble else np.array(list(zip(*metainds))).reshape(-1,2)#sum(list(zip(*metainds)), ())
    ...
    def get_labpath(imgpath, cls_name):
        if cfg.data == 'voc':
            ...
            if 'train2014' in imgpath:
                labpath = imgpath.replace('/train2014', '/labels_1c/train2014/{}'.format(cls_name)) \
                                 .replace('.jpg', '.txt').replace('.png','.txt')
            elif 'val2014' in imgpath:
                labpath = imgpath.replace('/val2014', '/labels_1c/val2014/{}'.format(cls_name)) \
                                 .replace('.jpg', '.txt').replace('.png','.txt')
            else:
...
if __name__ == '__main__':
    ...
    trainlist = '/home1/liqian/data/voc/voc_train.txt'
