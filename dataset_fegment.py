...
class listDataset(Dataset):
    ...
    def __init__(self, root,
        ...
        self.fpns=fpns
        self.n_cls = len(cfg.base_classes)
    ...
    def __getitem__(self, index):
        ...
        imgpath=imgpath.replace('JPEGImages','SegmentationClass')
        img=Image.open(imgpath)
        for wh in self.fpns[1:]:
            im=np.array(img.resize(wh,Image.ANTIALIAS),dtype=np.int16)
            im[im==255]=0
