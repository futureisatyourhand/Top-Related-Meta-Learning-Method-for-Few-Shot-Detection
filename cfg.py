...
###similar classes as the same
#same=open('cfg/groups.txt','r') #coco
same=open('cfg/sim.txt','r')#voc
sc=[]
for c in same.readlines():
    #print("====",c.strip('\n').split(','))
    sc.append(c.strip('\n').split(','))
__C.same_classes=sc
same.close()
...
def load_conv(buf, start, conv_model,train=False):
    num_w = conv_model.weight.numel()
    if train==True:
        if conv_model.bias is not None:
            num_b = conv_model.bias.numel()
            conv_model.bias.data.copy_(torch.cat((torch.from_numpy(buf[start:start+num_b//2]),torch.from_numpy(buf[start:start+num_b//2]))))
            start = start + num_b//2
        conv_model.weight.data.copy_(torch.cat((torch.from_numpy(buf[start:start+num_w//2]),torch.from_numpy(buf[start:start+num_w//2]))))
        start = start + num_w//2
    else:
        if conv_model.bias is not None:
            num_b = conv_model.bias.numel()
...
def load_conv_bn(buf, start, conv_model, bn_model,train=False):
    num_w = conv_model.weight.numel()
    ...
    if train==True:
        print("===num_w fpn feat==",num_w,num_b,num_w//2)
        value=torch.from_numpy(buf[start:start+num_w//2])
        conv_model.weight.data.copy_(torch.cat((value,value)));        start = start + num_w//2 
    else:
        print("===num_w==",num_w,num_b,)
        print(start,torch.from_numpy(buf).shape)
        print(torch.from_numpy(buf[start:]).shape)
        conv_model.weight.data.copy_(torch.from_numpy(buf[start:start+num_w])); start = start + num_w 
    return start
