...
class Darknet(nn.Module):
    ...
    def load_weights(self, weightfile,test=False):
            ...
            for block in blocks:
                ...
                if block['type'] == 'net' or block['type'] == 'learnet':
                    ...
                    if self.is_dynamic(block) and model[0].weight is None:
                        continue  
                    if test==False and (self.is_residual(block) or ind in [29,30,35,37,8,10,12]):
                        continue
                    if (flag6==1 and ind==6) or (ind==8):
                        continue
                    if ind==6:
                        flag6=1
    ...
    def is_residual(self,block):
        return 'residual' in block and int(block['residual'])==1
