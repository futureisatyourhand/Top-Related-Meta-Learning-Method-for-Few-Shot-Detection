...
class Darknet(nn.Module):
    def __init__(self, darknet_file, learnet_file,fpn_file):
        ...
        self.models = self.create_network(self.blocks,True) # merge conv, bn,leaky
        
        self.learnet_models = self.create_network(self.learnet_blocks,False)
        ...
        self.seen = 0
        self.rgbs=torch.FloatTensor([[[[0.25]],[[0.25]],[[0.5]]]]).cuda()
    ...
    def detect_forward(self,dynamic_weights,rgbs,*imgs):
        # Perform detection
        ind = -2
        dynamic_cnt = 0
        self.loss = None
            ...
            if block['type'] == 'net':
                ...
                if block['type']=='convolutional':
                    if self.is_dynamic(block):                    
                        ...
                        if conv_id in self.fpn_feat:                
                            #up=nn.functional.upsample(x,scale_factor=2, mode='bilinear')
                            #print(self.rgbs.shape,imgs[flag_fpn].shape)
                            #print("=====",torch.sum(imgs[flag_fpn]*self.rgbs,dim=-1).shape)

                            #aa=torch.sum(imgs[flag_fpn]*rgbs.float(),dim=1).unsqueeze(1)

                            #up=up*aa
                            
                            x=torch.cat((self.fpn_models[flag_fpn-1](imgs[flag_fpn]),x),dim=1)
                            #x=self.fpn_models[flag_fpn-1](imgs[flag_fpn])+x
                            flag_fpn+=1  
    ...
    def forward(self,metax, mask,rgbs, *data):
        # pdb.set_trace()
        dynamic_weights = self.meta_forward(metax, mask)
        
        x,dynamic_meta = self.detect_forward(dynamic_weights,rgbs,*data)
        return x,dynamic_meta
    ...
    def create_fpn(self,blocks):
        ...
        for block in blocks:
            if block['type']=='prev_filters':
                ...
                layers.append(int(block['feat_layer']))
                ...
                if batch_normalize:
                    model.add_module(
                        ...
                        Conv2d(3, prev_filters[conv_id], kernel_size, stride, pad, groups=groups, bias=False))
                    model.add_module(
                        'bn{0}'.format(conv_id),
                        nn.BatchNorm2d(prev_filters[conv_id]))
                    #model.add_module('bn{0}'.format(conv_id), BN2d(filters))
                else:
                    model.add_module(
                        'conv{0}'.format(conv_id),
                        Conv2d(3, prev_filters[conv_id], kernel_size, stride, pad, groups=groups, bias=bias))
                if activation == 'leaky':
    ...
    def create_network(self, blocks,fpn=False):
        models = nn.ModuleList()
        ...
        for block in blocks:
            if block['type'] == 'net' or block['type'] == 'learnet':
                ...
                if (fpn==True) and (conv_id in self.fpn_feat):                
                    prev_filters=prev_filters+self.fpn_layers[self.fpn_feat.index(conv_id)]
    ...
    def load_weights(self, weightfile,train=False):
        ...
        #block_model=[(self.blocks, self.models), (self.learnet_blocks, self.learnet_models)]##new version
        #self.init_models()
            ...
            for block in blocks:
                ...
                if block['type'] == 'net' or block['type'] == 'learnet':
                    ...
                    if self.is_dynamic(block) and model[0].weight is None:
                        ...
                        continue    
                    batch_normalize = int(block['batch_normalize'])
                    if (cnt in self.fpn_feat) and train==True:
                        flag=True
                    else:
                        flag=False
                    if batch_normalize:                        
                        start = load_conv_bn(buf, start, model[0], model[1],flag)
                    else:
                        start = load_conv(buf, start, model[0],flag)
                    ...
                    print('unknown type %s' % (block['type']))
            i+=1
        ###old version   train==False
        ##new version train==True
        if train==False:
            j=0
            for i in range(len(self.fpn_blocks)):
                block=self.fpn_blocks[i]
                if block['type'] == 'prev_filters':
                    continue
                else:                    
                    model = self.fpn_models[j]
                    if model[0].weight is None:
                        continue    
                    batch_normalize = int(block['batch_normalize'])
                    if batch_normalize:                        
                        start = load_conv_bn(buf, start, model[0], model[1],train)
                    else:

                        start = load_conv(buf, start, model[0],train)
    ...
    def save_weights(self, outfile, cutoff=0):
        # pdb.set_trace()
        if cutoff <= 0:
            cutoff = len(self.blocks) - 1 + len(self.learnet_blocks)#+len(self.fpn_blocks)-1
        ...
        ###将图像fpn的权重进行存储
        for b in range(len(self.fpn_blocks)-1):
            block = self.fpn_blocks[b+1]
            model = self.fpn_models[b]            
            if block['type'] == 'convolutional':                
                if model[0].weight is None:
                    continue
                print("=============save================")
    ...
    def init_models(self):
        for models in [self.fpn_models]:
            for model in models:
                ...
                elif isinstance(model, nn.LeakyReLU):
