...
from get_weights import Darknet
import dataset
import torch
...
def valid(datacfg, darknetcfg, learnetcfg,fpncfg, weightfile, outfile, use_baserw=False):
    ...
    #prefix = weightfile.split('/')[0]+"/"+weightfile.split('/')[1]+"/"+weightfile.split('/')[2]+"/" + backup.split('/')[-1]+"_test" + ckpt_pre + ckpt
    prefix="results/" + backup+"_test_plot"+ ckpt_pre + ckpt
    ...
    m = Darknet(darknetcfg, learnetcfg,fpncfg)
    m.print_network()
    ...
    if False:
        ...
        if True:
            ...
            f = 'data/rws/voc_novel{}_.pkl'.format(2)
    ...
    fps = [0]*n_cls
    #fps_ious=[0]*n_cls
    print(metaset.classes)
    for i, cls_name in enumerate(metaset.classes):
        buf = '%s/%s%s.txt' % (prefix, outfile, cls_name)
        fps[i] = open(buf, 'w')
        
        #ious_f='%s/ious_%s%s.txt' % (prefix, outfile, cls_name)
        #fps_ious[i]=open(ious_f,'w')
    ...
    for batch_idx, (data,target) in enumerate(valid_loader):
        ...
        data = Variable(data[0].cuda(), volatile = True)
        for b in range(data.shape[0]):
            lineId = lineId + 1
            imgpath = valid_dataset.lines[lineId].rstrip()
            imgid = os.path.basename(imgpath).split('.')[0]
            if '005767' not in imgid:
                continue
            #print(data.shape,imgid)
            #np.save("plot/atlas_data/"+str(imgid)+".npy",np.expand_dims(data.data[b].cpu().numpy(),axis=0))
        #rgbs=Variable(rgbs.cuda(),requires_grad=False)
        #print("===output===",data.shape)  
        #data=Variable(torch.ones([1,3,416,416]).float().cuda(),volatile=True)
        #start=time.time()
        ##############np.save("convert_data/data_"+str(batch_idx)+".npy",data.cpu().numpy()) 
        #continue
        print(m.anchors)
        output,weights = m.detect_forward(dynamic_weights,data)
        #exit()
        #output=output.permute([0,2,1,3]).contiguous()
        for i in range(90,180):
            print("============",output.data.view([-1])[i],i)
        exit()
        #print("time:",float(round((time.time()-start)*1000)))
        #total+=time.time()-start
        #continue
        #print(output[0].unsqueeze(0).data.cpu().numpy())
            ...
            for i in range(n_cls):
                ...
                for box in boxes:
                    ...
                    for j in range((len(box)-5)/2):
                        ...
                        fps[i].write('%s %f %f %f %f %f %f\n' % (imgid,cls_conf, prob, x1, y1, x2, y2))
                        #fps_ious[i].write('%s %f %f %f %f %f %f\n' % (imgid,cls_conf,prob, x1, y1, x2, y2))
        #total+=time.time()-start
        #exit()
    print("time:",total,float(round(total*1000))/(nums*valid_batchsize))
    for i in range(n_cls):
        fps[i].close()
        #fps_ious[i].close()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) in [5,6,7]:
        ...
        for w in os.listdir(weight):
            ...
            valid(datacfg, darknet, learnet,fpn, weightfile, outfile, use_baserw)
    else:
        print('Usage:')
