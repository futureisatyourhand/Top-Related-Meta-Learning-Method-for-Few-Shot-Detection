...
def dynamic_conv2d(is_first, partial=None):
    ...
    class DynamicConv2d(_ConvNd):
        ...
        def forward(self, inputs):
            ...
            #print("==========dyanmic weights=========",dynamic_weight.shape)
            a=input
            ...
            conv_rlt = F.conv2d(input, dynamic_weight, self.bias, self.stride,
                            self.padding, self.dilation, groups)
            #print("dyconv time:",time.time()-s)
            d=dynamic_weight.permute([1,0,2,3])
            
            #t=time.time()
            #f=input*d
            #print(f.data,conv_rlt.data)
            #print("our time:",time.time()-s)
            feat_size = (conv_rlt.size(-2), conv_rlt.size(-1))
            #s=time.time()
            conv_rlt = conv_rlt.view(-1, n_channels, *feat_size)
            #print(conv_rlt[0],d)
            #print("dyconv2 time:",time.time()-s)
            #s=time.time()
            #f=torch.reshape(f,[-1,n_channels,feat_size[0],feat_size[1]])
            #print("our2 time:",time.time()-s)
            #s=time.time()
            return conv_rlt
